...
b <- lm(oralscore ~ smoker, data = pdata)
summary(b)
```
...
lm(oralscore ~ smoker, data = filter(pdata, study_name == "JieZ_2017")) %>%
  summary()
```

```{r}
summary(lm(oralscore ~ smoker + age, data = filter(pdata, study_name=='KeohaneDM_2020')))
```

```{r}
studies_without_age <- c("JieZ_2017")
studies_with_age <- c("KeohaneDM_2020", "SankaranarayananK_2015")
```
